...
- **Package Manager**: Prefer `bun` over `npm` for frontend tasks when available
...
- **Run Go linting**: ALWAYS use `cd src && golangci-lint run` directly (NOT task test:lint)
...
- **IMPORTANT**: Prefer direct commands over task wrappers when possible
...
- **API Changes**: All changes to the REST API _must_ be reflected in `api/v2.0/swagger.yaml`. Use `task build:gen-apis` to regenerate the server code after making changes.
- **Database Migrations**: Database schema changes require a new migration file in `make/migrations/postgresql/`.
- **Code Style**: Follow the existing code style. ALWAYS use `cd src && golangci-lint run` directly for linting (NOT task wrappers).
- **Direct Commands**: Prefer using direct commands (go test, golangci-lint, bun) over task wrappers when possible for faster execution and clearer output.
- **Commit Messages**: ALWAYS use concise commit messages. Focus on what changed and why in 1-2 sentences. Avoid verbose explanations.
...
## FederatedIdP & RobotJWT Testing

Tests for the Federated Identity Provider feature require a running PostgreSQL database.

### Prerequisites

```bash
# Start dev infrastructure (PostgreSQL, Redis, Registry)
task dev:infra:up

# Verify PostgreSQL is accessible
PGPASSWORD=root123 psql -h localhost -p 5432 -U postgres -d registry -c "SELECT 1;"
```
...
# FederatedIdP DAO tests (requires migrations symlink)
cd pkg/federatedidp/dao
ln -sf ../../../../make/migrations migrations
go test -v .

# RobotJWT middleware tests
cd ../../../server/middleware/security
ln -sf ../../../../make/migrations migrations
go test -v . -run "RobotJWT"

# All FederatedIdP tests
cd ../../..
go test -v ./pkg/federatedidp/...
```
...
```bash
cd src
golangci-lint run ./pkg/federatedidp/...
golangci-lint run ./server/middleware/security/...
```

### Key Test Files

- `src/pkg/federatedidp/dao/dao_test.go` - DAO layer tests including `FindMatchingRobot` scenarios
- `src/server/middleware/security/robotjwt_test.go` - JWT authentication middleware tests
- `testTopMatchrobot.md` - Test scenario documentation
